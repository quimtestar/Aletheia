...
@TaggedCommand(tag = "unfbd", groupPath = "/statement", factory = UnfoldingContextsByDeclaration.Factory.class)
public class UnfoldingContextsByDeclaration extends TransactionalCommand
{
	private final Declaration declaration;

	public UnfoldingContextsByDeclaration(CommandSource from, Transaction transaction, Declaration declaration)
	{
		super(from, transaction);
		this.declaration = declaration;
	}

	protected Statement getDeclaration()
	{
		return declaration;
	}
	...
	protected RunTransactionalReturnData runTransactional()
	{
		...
		for (UnfoldingContext unf : declaration.unfoldingContexts(getTransaction()))
		{
			List<? extends Statement> path = unf.statementPath(getTransaction(), getActiveContext());
			list.add(path);
		}
	...
	public static class Factory extends AbstractVoidCommandFactory<UnfoldingContextsByDeclaration>
	{
		...
		@Override
		public UnfoldingContextsByDeclaration parse(CommandSource from, Transaction transaction, Void extra, List<String> split) throws CommandParseException
		{
			...
			if (!(statement instanceof Declaration))
				throw new CommandParseException("Statement not declaration:" + split.get(0));
			return new UnfoldingContextsByDeclaration(from, transaction, (Declaration) statement);
		}
		...
		{
			return "Lists the unfolding contexts that unfold the given declaration.";
		}
