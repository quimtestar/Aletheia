...
public class MySplashStartupProgressListener extends SplashStartupProgressListener implements StartupProgressListener
{
	private final JWindow window;

	private float progress = 0;

	public MySplashStartupProgressListener()
	{
		JWindow window = null;
		try (InputStream is = ClassLoader.getSystemResourceAsStream("aletheia/gui/app/splash.png"))
		{
			...
			JPanel panel = new JPanel()
			{
				...
				protected void paintComponent(Graphics graphics)
				{
					...
					int position = (int) (progress * (MySplashStartupProgressListener.this.window.getSize().width - 4));
					graphics.fillRect(2, 466, position, 6);
					graphics.clearRect(position, 466, MySplashStartupProgressListener.this.window.getSize().width - 4, 6);
