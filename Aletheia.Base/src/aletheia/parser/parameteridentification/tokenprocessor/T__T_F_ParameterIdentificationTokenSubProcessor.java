...
public class T__T_F_ParameterIdentificationTokenSubProcessor extends ParameterIdentificationTokenSubProcessor
{
	...
	@Override
	protected ParameterIdentification subProcess(ParseTree token) throws TokenProcessorException
	{
		ParameterIdentification head = getProcessor().processParameterIdentification((ParseTree) token.getChildren().get(0));
		ParameterIdentification tail = getProcessor().processParameterIdentification((ParseTree) token.getChildren().get(1));
